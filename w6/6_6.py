...
def LCS(v, w):
	...
	for i in xrange(1, v_len):
		...
		for j in xrange(1, w_len):
			...
			if(res[i][j] <= 0):
				res[i][j] = 0
				backtrack[i][j] = DISTINGUISHER
			else:
				# Backtrack matrix - key:   '0' = down   '1' = right   '2' = diagonal
				backtrack[i][j] = 0 if res[i][j] == res[i - 1][j] + score(v_i, '-') else (1 if res[i][j] == res[i][j - 1] + score('-', w_j) else 2)

	# find the max value in the entire res
	max_i = 0
	max_j = 0
	max_res = 0
	...
	print_alignment(v, w, backtrack, max_i, max_j)

def print_alignment(v, w, backtrack, max_i, max_j):
	i_idx = max_i
	j_idx = max_j
	res_v = []
	res_w = []
	...
	while i_idx > 0 or j_idx > 0:
		if backtrack[i_idx][j_idx] == 2:
			...
			res_w.append(w[j_idx - 1])
			j_idx -= 1
		elif backtrack[i_idx][j_idx] == 0:
			res_v.append(v[i_idx - 1])
			res_w.append('-')
			i_idx -= 1
		elif backtrack[i_idx][j_idx] == DISTINGUISHER:
			break
